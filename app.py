# DirectMarket — Pro Dashboard (Cloud-safe)
# Clean layout, logos, trending, price↔sentiment, watchlist.
...
# Small, rounded logos in tables (safety if style.css missing)
st.markdown("""
<style>
[data-testid="stDataFrame"] img{height:24px !important;width:auto !important;border-radius:6px;}
</style>
...
def get_logo_url(ticker: str) -> str:
    """Try yfinance logo_url; fallback to Clearbit via company website."""
    info = {}
    try:
    ...
    if isinstance(website, str) and website:
        try:
            ...
            if host:
                return f"https://logo.clearbit.com/{host}?size=40"
        except Exception:
            pass
    return ""
...
def price_pulse_chart(price_df: pd.DataFrame, senti_df: pd.DataFrame, ticker: str):
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=price_df["Datetime"], y=price_df["Close"], mode="lines", name=f"{ticker} Price"))
    fig.add_trace(go.Scatter(x=senti_df["Datetime"], y=senti_df["sentiment"], mode="lines",
                             name="Pulse (0–100)", yaxis="y2"))
    fig.update_layout(
...
with nv1:
    st.markdown(
        ...
        "<div><h2>DirectMarket <span class='mp-badge'>PRO</span></h2>"
...
with tab_overview:
    ...
    # --- Trending (full width, fixed height) ---
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "pulse": st.column_config.ProgressColumn("pulse", min_value=0, max_value=100, format="%.0f"),
        },
    )
    ...
    with a1: ticker = st.text_input("Search ticker", value=default_ticker).upper().strip()
    ...
    if not st.session_state.watchlist:
        ...
        with st.spinner("Refreshing quotes…"):
            for t in st.session_state.watchlist:
                ...
                if df.empty:
                    rows.append({"logo": get_logo_url(t), "ticker": t,
                                 "price": np.nan, "change_%": np.nan, "pulse": np.nan})
                else:
                    ...
                    rows.append({"logo": get_logo_url(t), "ticker": t,
                                 "price": latest, "change_%": pct, "pulse": pulse})
        wl = pd.DataFrame(rows)

        # Safe text mapping (no ambiguous truth values)
...
# ===== ABOUT =====
with tab_about:
    st.subheader("About DirectMarket")
    st.markdown(
        "- **What it does:** surfaces trending tickers from social chatter and compares **sentiment vs. price**.\n"
        "- **How it works:** price via Yahoo Finance; sentiment is demo until Live data is connected.\n"
        "- **What’s next:** Live Reddit mentions, alerts, CSV upload for portfolio view, and a public marketing site."
    )
    st.caption("Information is provided for educational purposes only and is not financial advice.")
